...
withr::local_options(joyn.match_type = "m:m")
...
The output table is the result of a *full join* -which is what `joyn` executes by default. This means that it retains both matching and non matching rows from both `x1` and `y1`. In addition, the resulting table contains the `.joyn` variable, which is the *reporting variable*. (see below ⬇️)
