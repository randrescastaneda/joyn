...
One of the value added of `joyn` is that it produces a number of messages that are intended to inform you about the status of the join. The display of such messages controlled by the argument `verbose`, which allows you to show (`verbose = TRUE`) or silent (`verbose = FALSE`) any messages.

To further explore messages in `joyn`, please refer to the [TODO: ADD LINK] **messages article**.
