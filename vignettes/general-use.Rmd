...
have a few tables lazy-loaded in memory, x1, x2, x3, x4, y1, y2, y3, y4.
...
3, "x & y", "Matching obs available in both",
...
6, "not updated", "Actual values and NAs in x are NOT updated actual values in y"
...
Join type refers to the correspondence that exists between the observations of
the joining tables. Following Stata's convention, we could have four different
join types, one-to-one (1:1) , one-to-many (1:m), many-to-one (m:1), and
...
care about join types or you don't think it is necessary to use them for your
