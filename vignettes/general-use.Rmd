...
is `joyn(x1, y1)[]` instead of `joyn(x1, y1)`.
...
joyn(x1, y1, by = "id", match_type = "m:m")[]
```
...
joyn(x4, y4, by = c("id1 = id"), match_type = "m:m")

# joining by id1 in x and id in y, and id2  in both
joyn(x4, y4, by = c("id1 = id", "id2"), match_type = "m:m")
```
...
-   `keep = "left"` or `keep = "master"` keeps all observations in x, regardless
    ...
    ```{r}
    # keep obs in x
    joyn(x1, y1, keep = "left", match_type = "m:m")
    ```
...
-   `keep = "right"` or `keep = "using"` keeps all observations in y, regardless
    ...
    ```{r}
    # keep obs in y
    joyn(x1, y1, keep = "right", match_type = "m:m")
    ```
