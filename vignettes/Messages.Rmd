...
Also, an info message will let you know that the name you are trying to assign to the reporting variable is already present in the returning table, so that it will be changed by adding a suffix to it.
...
Furthermore, info messages will help you being aware of which variables in `y` will be kept after the merging. For example, recall that you can specify which variables from `y` to retain in the returning table. If any of these variable names is in the `by` variables, `joyn` will notify you that they will be removed from the `y_vars_to_keep`.
