...
In `joyn`, you can also identify variables which possibly uniquely identify your data table `x` using the `possible_ids()` function. For example,
...
Additionally, `joyn` makes available the `is_balanced()` function to the user. This is instrumental in assessing the completeness/balance(?) of the data table within a specified group. By default, `is_balanced()` will tell you if/if not the table is balanced. However, you can also set `return = "table"` to get a summary of the unbalanced observations. In other words, those combinations of elements between the variables that is not present in the input table.
...
Finally, `joyn` also provides a function for easily generating simple frequency tables, so that you can have an overview of the distribution of values within your data tables.
