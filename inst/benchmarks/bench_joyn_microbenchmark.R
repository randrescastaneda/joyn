...
#   JOYN_BENCH_REPS_SMALL=50        # default: 50
#   JOYN_BENCH_REPS_MEDIUM=50       # default: 50
#   JOYN_BENCH_REPS_LARGE=30        # default: 30
#   JOYN_BENCH_REPS_XL=30           # default: 30
...
parse_sizes <- function() {
  ...
  if (!nzchar(value)) {
    return(as.integer(c(5e4, 1e5, 5e5)))
  }
  ...
  if (any(is.na(sizes)) || any(sizes < 1)) {
    stop(
      "JOYN_BENCH_SIZES must be comma-separated positive row counts.",
      call. = FALSE
    )
  }
  ...
  if (any(sizes > .Machine$integer.max)) {
    stop(
      "JOYN_BENCH_SIZES values exceed maximum integer (",
      .Machine$integer.max,
      ").",
      call. = FALSE
    )
...
make_data <- function(n, overlap = 0.8, seed = 42L) {
  ...
  if (shared_n < 1L || shared_n >= n) {
    stop(
      "Overlap yields invalid shared key count for n = ",
      n,
      ".",
      call. = FALSE
    )
...
dt_merge <- local({
  merge_data_table <- get(
    "merge.data.table",
    envir = asNamespace("data.table"),
    inherits = FALSE
  )

  function(x, y, by, all = FALSE, all.x = FALSE, all.y = FALSE) {
    merge_data_table(
      x = data.table::copy(x),
      y = data.table::copy(y),
      by = by,
...
reps_for_size <- function(n) {
  if (n <= 1e4) {
    return(parse_int("JOYN_BENCH_REPS_SMALL", 50L))
  }
  if (n <= 1e5) {
    return(parse_int("JOYN_BENCH_REPS_MEDIUM", 50L))
  }
  if (n <= 5e5) {
    return(parse_int("JOYN_BENCH_REPS_LARGE", 30L))
  }
  parse_int("JOYN_BENCH_REPS_XL", 30L)
}
...
join_grid <- function() {
  data.frame(
    ...
    engine = rep(
      c("collapse", "dplyr", "joyn", "data.table", "base"),
      times = 5L
    ),
...
build_expression <- function(join_type, engine) {
  switch(
    ...
    joyn = switch(
      join_type,
      left = quote(left_join(
        data$x_dt,
        data$y_dt,
        by = data$by,
        verbose = FALSE
      )),
      right = quote(right_join(
        data$x_dt,
        data$y_dt,
        by = data$by,
        verbose = FALSE
      )),
      full = quote(full_join(
        data$x_dt,
        data$y_dt,
        by = data$by,
        verbose = FALSE
      )),
      inner = quote(inner_join(
        data$x_dt,
        data$y_dt,
        by = data$by,
        verbose = FALSE
      )),
      anti = quote(anti_join(
        data$x_dt,
        data$y_dt,
        by = data$by,
        verbose = FALSE
    ...
    `data.table` = switch(
      join_type,
      left = quote(dt_merge(data$x_dt, data$y_dt, by = data$by, all.x = TRUE)),
      right = quote(dt_merge(data$x_dt, data$y_dt, by = data$by, all.y = TRUE)),
      full = quote(dt_merge(data$x_dt, data$y_dt, by = data$by, all = TRUE)),
      inner = quote(dt_merge(data$x_dt, data$y_dt, by = data$by)),
      anti = quote(data.table::copy(data$x_dt)[
        !data.table::copy(data$y_dt),
        on = data$by
      ]),
      stop("Unsupported join type: ", join_type, call. = FALSE)
    ),
    base = switch(
      ...
      inner = quote(base::merge(
        data$x_df,
        data$y_df,
        by = data$by,
        sort = FALSE
      )),
      anti = quote(data$x_df[
        is.na(match(data$x_df[[data$by]], data$y_df[[data$by]])),
        ,
        drop = FALSE
      ]),
      stop("Unsupported join type: ", join_type, call. = FALSE)
...
collect_measurements <- function(result, join_type, engine, n, times) {
  measurements <- as.data.frame(result, stringsAsFactors = FALSE)
  ...
  data.frame(
    ...
    expr = as.character(measurements$expr),
    time = as.numeric(measurements$time),
    unit = "ms",
    stringsAsFactors = FALSE
  )
}

summarise_result <- function(
  result,
  join_type,
  engine,
  n,
...
write_artifacts <- function(results, measurements, metadata, outdir, run_id) {
  artifact <- list(
    metadata = metadata,
    results = results,
    measurements = measurements
  ...
  latest_measurements_csv <- file.path(
    outdir,
    "benchmark-measurements-latest.csv"
  )
  snapshot_rds <- file.path(
    outdir,
    paste0("benchmark-results-", run_id, ".rds")
  )
  snapshot_csv <- file.path(
    outdir,
    paste0("benchmark-results-", run_id, ".csv")
  )
  snapshot_measurements_csv <- file.path(
    outdir,
    paste0("benchmark-measurements-", run_id, ".csv")
  )

  saveRDS(artifact, latest_rds)
  utils::write.csv(results, latest_csv, row.names = FALSE)
  utils::write.csv(measurements, latest_measurements_csv, row.names = FALSE)
  saveRDS(artifact, snapshot_rds)
  utils::write.csv(results, snapshot_csv, row.names = FALSE)
  utils::write.csv(measurements, snapshot_measurements_csv, row.names = FALSE)

  log_line("saved artifact: ", latest_rds)
  log_line("saved artifact: ", latest_csv)
  log_line("saved artifact: ", latest_measurements_csv)
  log_line("saved snapshot: ", snapshot_rds)
  log_line("saved snapshot: ", snapshot_csv)
  log_line("saved snapshot: ", snapshot_measurements_csv)
...
measurements <- vector("list", length = nrow(specs) * length(sizes))
result_index <- 1L

for (n in sizes) {
  data <- make_data(n = n, overlap = overlap)
  times <- if (smoke) 5L else reps_for_size(n)

  log_line(
    "dataset ready: n=",
    format_size(n),
    " | shared keys=",
    format_size(data$shared_n),
    " | overlap=",
    sprintf("%.2f", data$overlap)
  )
  ...
  for (row_id in seq_len(nrow(specs))) {
    ...
    measurements[[result_index]] <- checkpoint$measurements
...
results_df <- results_df[
  order(results_df$join_type, results_df$engine, results_df$n),
  ,
  drop = FALSE
]
row.names(results_df) <- NULL

measurements_df <- data.table::rbindlist(
  measurements,
  use.names = TRUE,
  fill = TRUE
)
...
measurements_df <- measurements_df[
  order(measurements_df$join_type, measurements_df$engine, measurements_df$n),
  ,
  drop = FALSE
]
row.names(measurements_df) <- NULL
...
metadata <- list(
  ...
  repetitions = list(
    smoke = 5L,
    small = reps_for_size(1e4),
    medium = reps_for_size(1e5),
    large = reps_for_size(5e5),
    xl = reps_for_size(1e6)
  ),
  overlap = overlap,
